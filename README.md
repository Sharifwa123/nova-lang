...
procedures, `DATA` named types, persistence (`SAVE`/`GET`/`DELETE`), a
...
and validating a real JSON request body; v0.15 connects `PAGE` and
`SERVICE` for the first time — `nova serve` now serves compiled `PAGE`
HTML alongside the API, `BUTTON` works inside `FOR EACH`, and a click
